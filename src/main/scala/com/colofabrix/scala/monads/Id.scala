...
object Id {
  ...
  implicit class IdOps[A]( id: Id[A] ) {
    def map[B]( f: A => B )( implicit fi: Functor[Id] ): Id[B] =
      fi.fmap( id )( f )

    def flatMap[B]( f: A => Id[B] )( implicit mi: Monad[Id] ): Id[B] =
      mi.bind( id )( f )
  }
  ...
  implicit def monoidId[A: Monoid]: Monoid[Id[A]] = new Monoid[Id[A]] {
    ...
    override def mempty: Id[A] =
      Id( ma.mempty )

    override def mappend( x: Id[A], y: Id[A] ): Id[A] =
      Id( ma.mappend(x.value, y.value) )
  }
  ...
  implicit val monadId: Monad[Id] = new Monad[Id] {
    ...
    override def bind[A, B]( ma: Id[A] )( f: A => Id[B] ): Id[B] =
      f( ma.value )
  }
